...
from bot.database.methods.insert import add_static_user,add_payment
from bot.database.methods.update import (
    add_time_person,
    person_banned_true,
    update_balance_person, add_balance_person
)
...
@user_management_router.callback_query(F.data.startswith('confirmSBP_'))
async def confirm_pay_sbp(call: CallbackQuery, state: FSMContext) -> None:
    lang = await get_lang(call.from_user.id, state)
    user_id = call.data.split("_")[1]
    user_lang = await get_lang(user_id, state)
    ...
    if not await add_balance_person(
        ...
        return
    await call.answer(text="Баланс юзера пополнен",show_alert=True)
    await add_payment(tgid=int(user_id),deposit=int(amount), payment_system="СБП")
    await call.message.delete()
    await call.message.bot.send_message(chat_id=user_id,
        text=_('payment_success', user_lang).format(total_amount=amount)
    )


@user_management_router.callback_query(F.data.startswith('declineSBP_'))
async def decline_pay_sbp(call: CallbackQuery, state: FSMContext) -> None:
    lang = await get_lang(call.from_user.id, state)
    user_id = call.data.split("_")[1]
    user_lang = await get_lang(user_id, state)
    amount = call.data.split("_")[2]
    await call.answer(text="Пополнение отклонено",show_alert=True)
    await call.message.delete()
    await call.message.bot.send_message(chat_id=user_id,
        text=_('payment_declined', user_lang)
    )
